import React, { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { Plus } from "lucide-react";
import { getAllPublishers } from "../../../api/adminApi/managePublishersApi";

export default function ManagePublishers() {
    const [publishers, setPublishers] = useState([]);
    const [loading, setLoading] = useState(true);
    const navigate = useNavigate();

    useEffect(() => {
        fetchPublishers();
    }, []);

    const fetchPublishers = async () => {
        try {
            const data = await getAllPublishers();
            setPublishers(Array.isArray(data) ? data : []);
        } catch (err) {
            console.error("Error loading publishers:", err);
            setPublishers([]);
        } finally {
            setLoading(false);
        }
    };

    return (
        <div className="p-6 w-full">

            {/* Header + Button */}
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-semibold">Manage Publishers</h1>

                <button
                    onClick={() => navigate("/manage-publishers/add")}
                    className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 
                               text-white rounded-lg shadow"
                >
                    <Plus size={18} />
                    Add Publisher
                </button>
            </div>

            {loading ? (
                <p>Đang tải...</p>
            ) : (
                <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800 shadow-xl overflow-x-auto">
                    <table className="min-w-full text-sm">
                        <thead>
                            <tr className="bg-zinc-800/60 text-blue-300">
                                <th className="p-3 text-left pl-60">Publisher Name</th>
                                <th className="p-3 text-left">Address</th>
                            </tr>
                        </thead>

                        <tbody className="divide-y divide-zinc-800 text-zinc-300">
                            {publishers.map((pb) => (
                                <tr key={pb.publisherId} className="hover:bg-zinc-800/40 transition">
                                    <td className="p-3 pl-60">{pb.publisherName}</td>

                                    <td className="p-3 text-zinc-500">
                                        {pb.address ?? "—"}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
            )}
        </div>
    );
}
